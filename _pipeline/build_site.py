...
APEX='https://terpsdispensary.com'  # canonical host for blog (old Wix /post/<slug> URLs)
...
<div><h4>Shop</h4><a href="{prefix}menu.html">Full menu</a>{links}<a href="{prefix}blog/index.html">Blog</a></div>
    ...
    for p in posts:
        ...
        ld={"@context":"https://schema.org","@type":"Article","headline":p['title'],
            ...
            "publisher":{"@type":"Organization","name":"Terps Dispensary",
                         "logo":{"@type":"ImageObject","url":f"{APEX}/img/badge.png"}}}
        if hero: ld["image"]=f"{APEX}/img/blog/{p['slug']}/hero.jpg"
...
<span class="more">Read more →</span></div></a>"""
    ...
    h=head("Blog — Terps Dispensary Pueblo | Cannabis News & Guides",
           ...
           f'{APEX}/blog/',extra).replace('{CSS}','../css/style.css')
    body=f"""{header('../')}
...
def build_meta(posts):
    ...
    urls+=[f'{APEX}/blog/']+[f"{APEX}/post/{p['slug']}" for p in posts]
